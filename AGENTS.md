...
- `components/` — shared UI components (`Gauge.tsx` — plain-language wellbeing gauge, Phase 5; `SiteNav.tsx` — cross-page nav linking Companion/Check-in/Gauge, wired into `app/layout.tsx`, see Key resolved decisions below)
...
| RAG retrieval failure handling in chat (Option B) | ✅ Resolved 2026-09-23: `lib/retrieval/retrieve.ts` still throws on any `match_vault_chunks` failure (missing migration, unindexed vault, RLS, transient outage), but `app/api/chat/route.ts` now catches it before streaming starts, logs the real underlying error server-side (`console.error`, not a generic message), and streams `copy.chatRetrievalUnavailableMessage` instead — a warm, clinical-safe message (try again shortly + 988 pointer, tone matched to `lib/companion/crisis.ts`) rather than ever answering the caregiving question ungrounded. `app/api/onboard/route.ts` and `app/api/checkin/route.ts` are deliberately unchanged — their silent-degradation-on-retrieval-failure behavior stays as-is. Covered by `eval/test-retrieval-failure-handling.mjs`. |
| Cross-page navigation | ✅ Resolved 2026-09-24: `components/SiteNav.tsx` (Lamplight-styled, `next/navigation` `usePathname` for active-link marking) links `/`, `/checkin`, `/gauge-preview` and is wired into `app/layout.tsx`, so `/` and `/gauge-preview`'s `<main>` now use `flex-1` instead of `min-h-dvh` to avoid stacking two full-viewport heights under it. It renders nothing on `/checkin` — that route keeps its own `CheckinNav` (same three links/labels from `copy.ts`, Still Water–styled in `checkin.css`) rather than importing Lamplight tokens, per the checkin visual-isolation decision above. `gauge-preview` is now caregiver-reachable via nav (was previously direct-URL-only; still not the Phase 6 dashboard). Gotcha: `cn()`/`tailwind-merge` silently drops a custom `fontSize` token (e.g. `text-caption`) when combined with a `text-*` color class in the same call — it misclassifies both as the same color-utility group — so don't pass a custom Tailwind theme size key through `cn()` alongside a color class; build the className directly instead (see `SiteNav.tsx`). |
