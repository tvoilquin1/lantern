...
assert(classifyGaugeColor(AMBER_THRESHOLD) === 'green', `score at AMBER_THRESHOLD (${AMBER_THRESHOLD}) classifies green (boundary is exclusive on the amber side)`);
assert(classifyGaugeColor(AMBER_THRESHOLD - 0.01) === 'amber', 'score just below AMBER_THRESHOLD classifies amber');
assert(classifyGaugeColor(RED_THRESHOLD) === 'amber', `score at RED_THRESHOLD (${RED_THRESHOLD}) classifies amber (boundary is exclusive on the red side)`);
assert(classifyGaugeColor(RED_THRESHOLD - 0.01) === 'red', 'score just below RED_THRESHOLD classifies red');
...
const entry = (color) => ({ at: new Date().toISOString(), type: 'session', compositeScore: 2, color });
...
assert(noPenalty === GAUGE_MAX, 'no missed-gap or night-time signal yields the max behavioral score');
...
assert(gapPenalty < GAUGE_MAX, 'a multi-day gap since the last check-in lowers the behavioral score');
...
assert(bothPenalties <= gapPenalty && bothPenalties <= nightPenalty, 'combined signals lower the score at least as much as either alone');

// ─── 6. buildScoreHistoryEntry ──────────────────────────────────────────────
console.log('\n[6] buildScoreHistoryEntry — shape');

const built = buildScoreHistoryEntry({ type: 'session', compositeScore: 4, color: 'green', sessionId: 'abc' });
assert(typeof built.at === 'string' && !Number.isNaN(Date.parse(built.at)), 'entry has a valid ISO "at" timestamp');
assert(built.type === 'session' && built.compositeScore === 4 && built.color === 'green', 'entry preserves the passed-in fields');
...
function makeMockSupabase(initialRow) {
  ...
  return {
    from(table) {
      assert(table === 'caregiver_state', `aggregateSessionSignals reads/writes the caregiver_state table (got "${table}")`);
      return {
        select: () => ({
          limit: () => ({
...

{
  ...
  const expectedComposite =
    5 * SIGNAL_WEIGHTS.selfReportSentiment + GAUGE_MAX * SIGNAL_WEIGHTS.behavioral + 5 * SIGNAL_WEIGHTS.lcwsRescreen;
  assert(Math.abs(result.compositeScore - expectedComposite) < 0.001, 'composite score matches the documented SIGNAL_WEIGHTS formula');
  ...
  assert(mock.getUpdatePayload().score_history.length === 1, 'score_history gains exactly one entry per session');
}

{
  ...
  assert(result.level2ShouldSurface === false, 'a single red day does not yet trigger Level 2 (needs RED_STREAK_ESCALATION_DAYS)');
  assert(mock.getUpdatePayload().gauge_crossed_red_pending === true, 'gauge_crossed_red_pending is persisted for the next session to consume');
}

{
  // Bidirectional movement: pre-seed a red history, then have this session recover to green.
  const redEntry = { at: '2026-09-19T00:00:00.000Z', type: 'session', compositeScore: 1.5, color: 'red' };
  const mock = makeMockSupabase({
    id: 'state-3',
  ...
  assert(result.redStreakDays === 0, 'red streak resets to 0 once the gauge moves back to green (bidirectional, not monotonic decay)');
...

{
  ...
  assert(result.redStreakDays === RED_STREAK_ESCALATION_DAYS, `three consecutive red sessions reaches RED_STREAK_ESCALATION_DAYS (${RED_STREAK_ESCALATION_DAYS})`);
  assert(result.level2ShouldSurface === true, 'Level 2 human-support escalation triggers once the red streak reaches RED_STREAK_ESCALATION_DAYS');
  assert(mock.getUpdatePayload().level2_support_pending === true, 'level2_support_pending is persisted for the checkin route to consume and surface');
